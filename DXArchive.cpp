...
wchar_t* DXArchive::sJisToW(char* pchTextA, const int cchTextA)
{
...
const wchar_t *DXArchive::GetOriginalFileName( u8 *FileNameTable )
{
	return sJisToW((char *)(FileNameTable + *((u16 *)&FileNameTable[0]) * 4 + 4), MAX_PATH) ;
}
...
bool DXArchive::isHex(const char* input, int len)
{
	for (int i = 0; i < len; i++) {
...
int DXArchive::char2int(char input)
{
...
void DXArchive::printKey(unsigned char* Key)
{
	char hexstr[26];
	for (int j=0; j<12; j++) {
...
void DXArchive::KeyCreate( const char *Source, unsigned char *Key )
{
	...
	else
	{
		...
		if (Len == DXA_KEYSTR_LENGTH * 2 && isHex(Source, DXA_KEYSTR_LENGTH))
		{
			char *Src = (char *)Source;

			//If key is exactly DXA_KEYSTR_LENGTH * 2 length and hexadecimal then convert it from hexadecimal
			int i = 0;
			while(Src[0] && Src[1] && i < DXA_KEYSTR_LENGTH)
			{
				Key[i++] = (unsigned char)(char2int(Src[0]) << 4 | char2int(Src[1]));
				Src += 2;
			}

			return;
		}
		else if( Len > DXA_KEYSTR_LENGTH )
		{
		...
		do
		{
			...
			else
			{
				...
				if( File.DataSize != 0 )
				{
					...
					// サイズを得る
					fseeko64( SrcP, 0, SEEK_END ) ;
					FileSize = ftello64( SrcP ) ;
					fseeko64( SrcP, 0, SEEK_SET ) ;
...
int DXArchive::DirectoryDecode( u8 *NameP, u8 *DirP, u8 *FileP, DARC_HEAD *Head, DARC_DIRECTORY *Dir, FILE *ArcP, unsigned char *Key )
{
	...
	if( Dir->DirectoryAddress != 0xffffffffffffffff && Dir->ParentDirectoryAddress != 0xffffffffffffffff )
	{
		...
		// ディレクトリの作成
		CreateDirectoryW( GetOriginalFileName( NameP + DirFile->NameAddress ), NULL ) ;

		// そのディレクトリにカレントディレクトリを移す
		SetCurrentDirectoryW( GetOriginalFileName( NameP + DirFile->NameAddress ) ) ;
	}
	...
	// 展開処理開始
	{
		...
		for( i = 0 ; i < Dir->FileHeadNum ; i ++, File = (DARC_FILEHEAD *)( (u8 *)File + FileHeadSize ) )
		{
			...
			else
			{
				...
				// ファイルを開く
				DestP = _wfopen( GetOriginalFileName( NameP + File->NameAddress ), L"wb" ) ;

				// データがある場合のみ転送
				if( File->DataSize != 0 )
				{
					// 初期位置をセットする
					if( ftello64( ArcP ) != ( s32 )( Head->DataStartAddress + File->DataAddress ) )
						fseeko64( ArcP, Head->DataStartAddress + File->DataAddress, SEEK_SET ) ;
				...
				// ファイルのタイムスタンプを設定する
				{
					...
					HFile = CreateFileW( GetOriginalFileName( NameP + File->NameAddress ),
				...
				// ファイル属性を付ける
				SetFileAttributesW( GetOriginalFileName( NameP + File->NameAddress ), ( u32 )File->Attributes ) ;
			}
		}
	}
...
int DXArchive::DecodeArchive( char *ArchiveName, char *OutputPath, const char *KeyString )
{
	...
	// ヘッダを解析する
	{
		...
		// ヘッダパックをメモリに読み込む
		fseeko64( ArcP, Head.FileNameTableStartAddress, SEEK_SET ) ;
...
int DXArchive::OpenArchiveFile( const char *ArchivePath, const char *KeyString )
{
	...
	// ヘッダを解析する
	{
		...
		// ヘッダパックをメモリに読み込む
		fseeko64( this->fp, this->Head.FileNameTableStartAddress, SEEK_SET ) ;
...
s64 DXArchive::LoadFileToMem( const char *FilePath, void *Buffer, u64 BufferLength )
{
	...
	if( FileH->PressDataSize != 0xffffffffffffffff )
	{
		...
		else
		{
			...
			// 圧縮データの読み込み
			fseeko64( this->fp, this->Head.DataStartAddress + FileH->DataAddress, SEEK_SET ) ;
...
DXArchiveFile::DXArchiveFile( DARC_FILEHEAD *FileHead, DXArchive *Archive )
{
	...
	if( FileHead->PressDataSize != 0xffffffffffffffff )
	{
		...
		// 圧縮データの読み込み
		fseeko64( this->Archive->GetFilePointer(), this->Archive->GetHeader()->DataStartAddress + FileHead->DataAddress, SEEK_SET ) ;
...
s64 DXArchiveFile::Read( void *Buffer, s64 ReadLength )
{
	...
	if( this->DataBuffer == NULL && ftello64( this->Archive->GetFilePointer() ) != (s32)( this->FileData->DataAddress + this->Archive->GetHeader()->DataStartAddress + this->FilePoint ) )
	{
		fseeko64( this->Archive->GetFilePointer(), this->FileData->DataAddress + this->Archive->GetHeader()->DataStartAddress + this->FilePoint, SEEK_SET ) ;
	}
